...
// --- EVENTO READY (LIMPA COMANDOS ANTIGOS E REGISTRA OS NOVOS) ---
client.on('ready', async () => {
    ...
    try {
        console.log('🔄 Removendo comandos antigos do servidor e globais...');

        // 1. Apaga comandos antigos específicos do seu servidor
        await rest.put(Routes.applicationGuildCommands(client.user.id, GUILD_ID), { body: [] });

        // 2. Apaga comandos antigos globais
        await rest.put(Routes.applicationCommands(client.user.id), { body: [] });

        console.log('🧹 Comandos antigos apagados com sucesso!');

        // 3. Registra apenas os novos comandos
        await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
        console.log('✅ Novos Slash Commands registrados com sucesso.');
    } catch (err) {
        console.error('❌ Erro ao atualizar os comandos:', err);
    }
});

// --- VOZ AUTOMÁTICA (CALL AUTO) ---
